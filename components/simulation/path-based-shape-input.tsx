...
export default function PathBasedShapeInput({ title, acceptedFormats, onShapeChange, onStartSimulation, workpieceWidth = 100, workpieceHeight = 80 }: PathBasedShapeInputProps) {
  ...
  const emitShape = useCallback((shape: unknown) => {
    setTimeout(() => {
      onShapeChange(shape)
    }, 0)
  }, [onShapeChange])
    ...
    <Card className="p-6 bg-card border border-border">
      ...
      <Tabs defaultValue="upload" className="w-full">
        ...
        <TabsContent value="coordinates" className="space-y-5">
          <p className="text-sm text-muted-foreground">Enter or nudge X/Y values, then Add Point to build a path. Close Shape will link last point to first.</p>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-1">
                  <Button size="icon" variant="outline" onClick={() => setCurrentX(prev => snap(prev - step))}><ArrowLeft className="w-4 h-4" /></Button>
                  <Button size="icon" variant="outline" onClick={() => setCurrentX(prev => snap(prev + step))}><ArrowRight className="w-4 h-4" /></Button>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                />
                <div className="flex gap-1">
                  <Button size="icon" variant="outline" onClick={() => setCurrentY(prev => snap(prev + step))}><ArrowUp className="w-4 h-4" /></Button>
                  <Button size="icon" variant="outline" onClick={() => setCurrentY(prev => snap(prev - step))}><ArrowDown className="w-4 h-4" /></Button>
                </div>
              </div>
              <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-4 text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Input type="number" value={gridSize} onChange={e => setGridSize(Math.max(1, Number(e.target.value)))} className="h-9 w-20" />
                </div>
              </div>
              <div className="flex gap-2 flex-wrap">
                ...
                <Button
                  variant="ghost"
                  onClick={() => {
                    setCoordPoints([]); emitShape({ type: 'coordinates', points: [] })
                  }}
                >
                  ...
                  <ol className="space-y-1">
                    {coordPoints.map((pt, i) => (
                      <li key={i} className="flex justify-between">
                        <span>#{i + 1}</span>
                        <span className="text-muted-foreground">x: {pt.x.toFixed(2)} y: {pt.y.toFixed(2)}</span>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => {
              ...
              <div className="border border-border rounded-lg p-2 bg-background/40">
                <svg viewBox="0 0 600 300" className="w-full h-40">
                  ...
                  {showGrid && (() => {
                    ...
                    for (let x = 300; x <= 600; x += xStep) lines.push(<line key={`gxR${x}`} x1={x} y1={0} x2={x} y2={300} stroke="#1f2937" strokeWidth="0.5" />)
                    for (let x = 300 - xStep; x >= 0; x -= xStep) lines.push(<line key={`gxL${x}`} x1={x} y1={0} x2={x} y2={300} stroke="#1f2937" strokeWidth="0.5" />)
                    for (let y = 150; y <= 300; y += yStep) lines.push(<line key={`gyD${y}`} x1={0} y1={y} x2={600} y2={y} stroke="#1f2937" strokeWidth="0.5" />)
                    for (let y = 150 - yStep; y >= 0; y -= yStep) lines.push(<line key={`gyU${y}`} x1={0} y1={y} x2={600} y2={y} stroke="#1f2937" strokeWidth="0.5" />)
                  ...
                  {/* Path lines */}
                  {coordPoints.length > 1 && coordPoints.map((pt, i) => {
                  ...
                  {coordPoints.length > 2 && coordPoints[0] && coordPoints[coordPoints.length - 1] && (
                    (() => {
                      ...
                      return <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#6366f1" strokeWidth="1" strokeDasharray="4 3" />
                    })()
                  )}
                  {/* Points */}
                  {coordPoints.map((pt, i) => {
